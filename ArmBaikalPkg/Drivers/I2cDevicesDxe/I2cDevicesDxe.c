  ...
  )
{
  switch (HostIndex) {
  case 0:
    PcdSet64S (PcdI2c0BaseAddr, BaseAddress);
    PcdSet32S (PcdI2c0Size, (UINT32)Size);
    break;
  default:
    ;
  }
  ...
  if (Desc != NULL) {
    ...
    if (HostDesc->Next != NULL) {
      ...
      if (HostDesc->Next->Data.Devices != NULL) {
        ...
        I2cDevicesUpdatePcds ((*Desc == NULL) ? 0 : HostIndex + 1,
                              MmioRegion->BaseAddress, MmioRegion->Size);
        if (*Desc == NULL) {
          *Desc = HostDesc->Next;
        }
      } else {
  ...
  )
{
  ...
  if (PcdGet64 (PcdI2c0BaseAddr) == 0x0) {
    I2cHost = NULL;
    I2cDevicesParseFdt (&I2cHost);
    while (I2cHost != NULL) {
      Handle = NULL;
      Status = RegisterDevice (&gArmBaikalNonDiscoverableI2cMasterGuid,
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    if (!EFI_ERROR (Status) &&
        CompareGuid (Device->Type, &gArmBaikalNonDiscoverableI2cMasterGuid)) {
      Status = gBS->OpenProtocol (
                      HandleBuffer[Index],
                      &gEfiI2cEnumerateProtocolGuid,
                      (VOID **) &I2cEnumerate,
